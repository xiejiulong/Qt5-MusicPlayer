...
void MainWidget::on_btnClear_clicked()
{
	...
	QMessageBox::StandardButton btn;
	btn = QMessageBox::question(this, u8"提示", u8"此操作不可恢复！\n确定要清空吗？", QMessageBox::Yes | QMessageBox::No);
	if (btn == QMessageBox::Yes)
	{
		ui->playListWidget->musicList.clear();
		ui->playListWidget->refresh();
...
{
	//QMessageBox::StandardButton btn;
	//btn = QMessageBox::question(this, "提示", u8"此操作不可逆！\n确实要清空吗?", QMessageBox::Yes|QMessageBox::No);
	//if (btn == QMessageBox::Yes) {
...
{
	//QMessageBox::about(this,"关于",QStringLiteral("LightMusicPlayer | 一款精致小巧的本地音乐播放器\n"
	//                               "作者：NJU-TJL\n\n"
	...
	QMessageBox::about(this, u8"关于", u8"轻量播放器");
}